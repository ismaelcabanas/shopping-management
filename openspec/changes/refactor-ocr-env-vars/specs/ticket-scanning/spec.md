...
**BREAKING CHANGE**: Environment variable names have been changed to be provider-agnostic:

- `VITE_GEMINI_API_KEY` → `VITE_OCR_LLM_API_KEY`
- `VITE_GEMINI_MODEL` → `VITE_OCR_LLM_MODEL`

Users must update their `.env` files:
1. Rename the API key variable to the new name
2. Rename the model variable to the new name (if customized)
3. The values remain the same - only the variable names change
4. For Gemini users, the API key is still obtained from https://makersuite.google.com/app/apikey